...
business_filtered$address[9]
```

```{r}
# Find out which columns have NAs
na_count <- colSums(is.na(business_filtered))
print(na_count)
...
library(dplyr)
clean_data <- business_filtered %>%
   filter(!is.na(address) & !is.na(postal_code) & !is.na(categories) & !is.na(attributes))

```
  
business_filtered <- read.csv("yelp_academic_dataset_business.csv", na.strings = c("", "NA" ))
