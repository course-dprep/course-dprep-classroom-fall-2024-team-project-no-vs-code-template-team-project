...
fix_json <- function(x) {
  ...
  x <- gsub("'", '"', x)
  # Fix improperly escaped quotes (handle any erroneous ""no"" cases)
  x <- gsub('""', '"', x)
  # Remove improperly placed double quotes inside values
  x <- gsub('"([^"]*)""', '"\\1"', x)
  # Fix the inner dictionaries by handling any misplaced quotes around braces
  x <- gsub('"{', '{', x)
  x <- gsub('}"', '}', x)
  return(x)
}

# Apply fix_json function to attributes in data_business
data_business$attributes <- data_business$attributes %>% 
  map(~ ifelse(!is.na(.x), fix_json(.x), NA))

# Safely parse the JSON strings into lists
data_business$attributes <- map(data_business$attributes, ~ tryCatch(fromJSON(.x, simplifyVector = TRUE), error = function(e) NULL))

# Get all unique attribute names
all_attributes <- data_business$attributes %>% 
  map(~ names(.x)) %>% 
  unlist() %>% 
  unique()
...
for (attribute in all_attributes) {
  data_business[[attribute]] <- map_int(data_business$attributes, ~ ifelse(is.null(.x[[attribute]]), 0, ifelse(.x[[attribute]] == "True", 1, 0)))
}
